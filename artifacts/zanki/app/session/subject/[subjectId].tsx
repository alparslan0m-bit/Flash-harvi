...
export default function SubjectSessionScreen() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useFlashcards(undefined, subjectId);
  ...
  const initialCards = useMemo(() => {
    ...
    const prepared = prepareSessionCards(allCards, userCards ?? [], {
      mode: "study",
      shuffle: true,
    });
    return prepared;
  ...
  const progressStyle = useAnimatedStyle(() => ({
    width: withSpring(`${(completedCount / totalCards) * 100}%`, {
      damping: 20,
    }),
  }));
  ...
  const handleRate = useCallback(
    ...
    [currentCard, queue, sessionRatings, flipProgress],
  );

  const finishSession = useCallback(
    async (finalRatings: Record<string, CardRating>) => {
      if (!user?.id || !subjectId || !allCards) return;
      setSubmitting(true);
      ...
      try {
        ...
        // Fallback to offline queue logic if needed
      }

      setSubmitting(false);
    },
    [user, subjectId, subjectName, userCards],
  );
  ...
  if (error || !initialCards || initialCards.length === 0) {
    return (
      <View style={[st.center, { backgroundColor: colors.background }]}>
        <Feather
          name={error ? "alert-circle" : "check-circle"}
          size={48}
          color={error ? colors.destructive : colors.success}
        />
        <Text style={[st.errTitle, { color: colors.foreground }]}>
          {error ? "Failed to load cards" : "You're all caught up!"}
        </Text>
        <Text style={[st.errMsg, { color: colors.mutedForeground }]}>
          {error ? (error as Error).message : "No cards due in this subject."}
        </Text>
        <TouchableOpacity
  ...
  if (finished) {
    ...
    return (
      <CardResultsView
        masteryRate={totalCount > 0 ? Math.round((goodCount / totalCount) * 100) : 0}
        againCount={ratingEntries.filter((r) => r === "again").length}
        hardCount={hardCount}
        goodCount={goodCount}
        totalCount={totalCount}
        lectureName={subjectName}
  ...
  return (
    <View style={[st.root, { backgroundColor: colors.background }]}>
      ...
      <View style={[st.header, { paddingTop: topPad + 10 }]}>
        <TouchableOpacity
          onPress={handleHome}
          style={[st.backBtn, { backgroundColor: colors.muted }]}
          activeOpacity={0.75}
        >
          <Feather name="x" size={18} color={colors.foreground} />
        </TouchableOpacity>
        <View style={st.headerCenter}>
          <Text style={[st.progressText, { color: colors.foreground }]}>
            {completedCount} / {totalCards}
          </Text>
          {subjectName && (
            <Text
              style={[st.lecName, { color: colors.mutedForeground }]}
              numberOfLines={1}
            >
              {subjectName}
            </Text>
        ...
        >
          ...
          >
            <View style={st.cardHeader}>
              ...
              >
                <Text style={[st.faceBadgeText, { color: colors.primary }]}>
                  FRONT
                </Text>
              </View>
              {currentCard?.hint && (
                <View style={st.hintBadge}>
                  ...
                  />
                  <Text
                    style={[st.hintText, { color: colors.mutedForeground }]}
                  >
                    {currentCard.hint}
                  </Text>
                </View>
            ...
            <View style={st.cardContent}>
              <Text style={[st.faceText, { color: colors.foreground }]}>
                {currentCard?.front}
              </Text>
            </View>
            {!isFlipped && (
              <Animated.View
                ...
                style={[st.tapHint, { backgroundColor: colors.muted + "40" }]}
              >
                <Feather name="zap" size={13} color={colors.primary} />
                <Text style={[st.tapHintText, { color: colors.foreground }]}>
                  TAP TO REVEAL
                </Text>
              </Animated.View>
            )}
          ...
          >
            <View style={st.cardHeader}>
              ...
              >
                <Text style={[st.faceBadgeText, { color: colors.success }]}>
                  ANSWER
                </Text>
              </View>
            </View>
      ...
      <View style={st.ratingWrapper}>
        {isFlipped ? (
          ...
          <View style={st.ratingPlaceholder} />
        )}
      </View>
    </View>
  );
...
const st = StyleSheet.create({
  ...
  ratingWrapper: { paddingBottom: 32 },
  ratingRow: { flexDirection: "row", gap: 12, paddingHorizontal: 20 },
  ratingPlaceholder: { height: 120 },
});
